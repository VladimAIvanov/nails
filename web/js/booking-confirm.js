...
async function onConflict(err) {
  ...
  /* Кнопку гасим. Время занято, и нажимать её больше незачем: сервер откажет
     ещё раз. Активная кнопка под сообщением «это время уже занято» обещает
     то, чего сделать нельзя, — человек жмёт и получает второй отказ.

     Вернётся она сама: выбор окна из списка ниже перезагружает страницу,
     и всё начинается заново с живым удержанием. */
...
submit.addEventListener('click', async () => {
  ...
  try {
    ...
    if (err.status === 409) {
      /* Кнопку не возвращаем: этим займётся onConflict, и вернуть её
         должен выбор другого окна, а не сам отказ. */
      await onConflict(err);
    } else {
      /* Прочие отказы поправимы на месте — комментарий длиннее тысячи
         знаков, пропавшая связь, — поэтому кнопка остаётся живой. */
