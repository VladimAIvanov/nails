# Схема базы данных сервиса «Ноготочки»

Документ описывает, какие таблицы нужны сервису онлайн-записи бьюти-студии «Ноготочки», какие в них поля и как они связаны. Схема выведена из экранов прототипа и карты связей — ничего «на будущее», только то, что реально показывают или собирают экраны.

**Основание:** карта связей прототипа (`Карта связей.dc.html`) и исходники семи страниц — `Ноготочки - Сайт.dc.html`, `Вход.dc.html`, `Кабинет.dc.html`, `Клиент.dc.html`, `Админ-панель.dc.html`, `Варианты.dc.html`, а также данные-заглушки `assets/nogotochki-data.js` и `assets/nogotochki-admin-data.js`.
...
## Обязательный минимум проектной работы

Пять сущностей из задания и таблицы, в которых они живут. Остальные таблицы документа — задел сверх обязательного объёма.

| Сущность | Таблицы | Что гарантирует база |
|---|---|---|
| Пользователи | `users` (роль `client` / `master` / `admin`), `user_roles`, `sessions` | телефон и почта уникальны; у клиента обязателен телефон или внешний вход |
| Услуги | `service_categories`, `services` | длительность кратна 5 минутам; отключение — `is_active = 0`, строка не удаляется, пока на неё ссылаются записи |
| Мастера | `users` + `master_profiles`, связь с услугами — `master_services` | профиль мастера привязан только к пользователю с ролью `master`; отключённый мастер — `users.is_active = 0` |
| Расписание | `working_hours` (недельный график студии и мастеров), `time_off` (блокировки), `schedule_exceptions` (график на дату) | конец интервала позже начала |
| Записи клиентов | `appointments`, `appointment_status_log`, `slot_holds` | триггеры `appointments_no_overlap_insert` и `_update` не дают двум действующим записям одного мастера пересечься; цена и длительность копируются в запись; отмена — смена статуса, а не удаление |

Схема создаётся миграциями `server/migrations/001…014`. Данные студии «Ноготочки» — последние три: `012` — название, `013` — часы работы вт–сб 10:00–20:00, перенос за сутки, записи без перерыва, рубрика «Брови», `014` — контакты. Мастера, их графики, услуги и тестовые записи загружает `server/src/seed.js`.
...
Переключатель подписан «Новые окна **у Анны**» — то есть подписка всегда на конкретного мастера, а не на студию целиком. Это отдельная таблица `slot_subscriptions`, а не флажок.
...
| Настройки | Онлайн-запись вкл/выкл, ручное подтверждение, напоминание за 2 часа, уведомлять о новых записях, часы работы Пн–Сб и Вс, карточка бота «@nogotochki_bot · подключён» | `studio_settings`, `working_hours` |
...
| 3 | `service_categories` | Категории прайса: Маникюр, Педикюр, Наращивание, Дизайн, Брови |
...
| 18 | `slot_subscriptions` | Подписки «Новые окна у Анны» — на конкретного мастера |
...
| 22 | `slot_holds` | Кратковременные удержания времени на период оформления записи |
| 23 | `push_devices` | Устройства клиента для пуш-уведомлений |
| 24 | `recurring_series` | Правила регулярных записей: «каждые две недели по вторникам» |
| 25 | `calendar_feeds` | Персональные ссылки на ленту визитов в формате iCalendar |
| 26 | `inbox` | Уведомления, которые человек читает в кабинете |
...
| `full_name` | `TEXT` | да | | «Марина», «Ноготочки» |
...
**Внешний вход (миграция `010_external_login.sql`).** Добавлены два поля:

| Поле | Тип | Обяз. | Описание |
|---|---|---|---|
| `provider` | `TEXT` + CHECK | нет | Каким сервисом вошёл человек. Пока допустимо одно значение — `yandex` |
| `provider_id` | `TEXT` | нет | Как этот сервис называет человека у себя. Почта со временем меняется, идентификатор — нет |

Ограничения при этом поменялись:

- `users_client_needs_phone` стало `users_client_needs_contact`: клиентке нужен телефон **или** внешняя учётная запись. Яндекс телефона не отдаёт, а отдельно спрашивать его при первом входе мы не стали;
- добавлено `users_provider_pair`: пара «сервис и идентификатор» заполняется целиком или не заполняется вовсе;
- уникальный индекс `users_provider_idx (provider, provider_id) WHERE provider IS NOT NULL`: одна учётная запись Яндекса — один человек в сервисе.

`password_hash` менять не пришлось: он был необязательным с самого начала — так в базе живут гостьи, которых записала студия сама.

Таблицу пришлось пересобрать: изменить `CHECK` у существующего столбца в SQLite нельзя. На `users` ссылаются двадцать три таблицы, поэтому у прогона миграций появился особый режим — см. отметку «ПЕРЕСБОРКА ТАБЛИЦЫ» в начале файла миграции и `src/migrate.js`.
...
| `phone` | `TEXT` | да | | |
| `email` | `TEXT` | нет | | |
| `telegram_bot_username` | `TEXT` | нет | | `nogotochki_bot` |
...
| `hold_ttl_min` | `INTEGER` | да | | Сколько минут живёт удержание слота на время оформления. По умолчанию 10 |
...
| `allow_overlap` | `INTEGER` 0/1 | да | | `true` — осознанное наложение поверх занятого времени. Ставится только администратором, гасит проверку пересечения **для этой записи при её создании**. Дальше запись занимает время как обычная |
...
Третий переключатель профиля — «Новые окна у Анны» — здесь отсутствует намеренно: его состояние это наличие активной строки в `slot_subscriptions`. Флаг рядом с подпиской был бы вторым выключателем той же лампы, и при расхождении неясно, какой из них главный.
...
Переключатель в профиле называется «Новые окна **у Анны**» — подписка всегда на конкретного мастера. Булев флажок в `notification_prefs` на такое не отвечает: он не хранит, чьи именно окна ждут.
...
### 4.22 `slot_holds` — удержание времени на период оформления

Между «выбрала 18:00» и «нажала Записаться» проходит минута-другая, пока заполняются контакты. Без удержания окно успевает занять кто-то другой, и клиентка получает отказ на последнем шаге.

**Это не таблица заранее нарезанных слотов.** Строка появляется, когда конкретный человек выбрал конкретное время, живёт несколько минут и исчезает. Свободное время по-прежнему вычисляется; удержания лишь временно вычитаются наравне с записями и блокировками.

| Поле | Тип | Обяз. | Ключ | Описание |
|---|---|---|---|---|
| `id` | `INTEGER` | да | PK | |
| `token` | `TEXT` | да | U | Выдаётся клиенту, по нему удержание снимается или превращается в запись |
| `master_id` | `INTEGER` | да | FK → `master_profiles.user_id` | |
| `client_id` | `INTEGER` | нет | FK → `users.id` | Пусто у гостя: запись без регистрации разрешена |
| `starts_at` | `TEXT` ISO-8601 UTC | да | | |
| `duration_min` | `INTEGER` | да | | Суммарная длительность выбранных услуг вместе с уборкой |
| `ends_at` | `TEXT` ISO-8601 UTC | да | | Вычисляемое поле |
| `expires_at` | `TEXT` ISO-8601 UTC | да | | Момент истечения. `CHECK (expires_at > created_at)` |
| `created_at` | `TEXT` ISO-8601 UTC | да | | |

Два триггера защищают удержание от бессмыслицы: нельзя удержать время, которое уже удерживается, и нельзя удержать время, на котором стоит активная запись. Проверяются только живые удержания — истёкшие время не занимают.

**Истёкшие строки удаляются лениво**, перед каждым расчётом окон и перед созданием записи. Отдельный планировщик не нужен: строк всегда единицы, а запрос по индексу на `expires_at` дешёвый.

### 4.23 `push_devices` — устройства для пушей

| Поле | Тип | Обяз. | Ключ | Описание |
|---|---|---|---|---|
| `id` | `INTEGER` | да | PK | |
| `user_id` | `INTEGER` | да | FK → `users.id` | |
| `token` | `TEXT` | да | U | Токен устройства от Apple, Google или браузера |
| `platform` | `TEXT` + CHECK | да | | `ios` · `android` · `web` |
| `device_name` | `TEXT` | нет | | «Pixel», чтобы человек узнал своё устройство в списке |
| `is_active` | `INTEGER` 0/1 | да | | Отозванный токен не удаляется, а гасится |
| `last_seen_at` | `TEXT` ISO-8601 UTC | нет | | |
| `created_at` | `TEXT` ISO-8601 UTC | да | | |

Единственное место в схеме, где секрет хранится **не** хешем. Пароль и токен сеанса сравниваются, поэтому им достаточно хеша; токен устройства нужно предъявить целиком службе доставки пушей — из хеша его не восстановить.

### 4.24 `recurring_series` — регулярные записи

Правило, а не набор записей: «каждые две недели по вторникам в 18:00». Визиты создаются вперёд на несколько повторов и дальше живут самостоятельно — перенос или отмена одного не трогает остальные.
...
| `master_id` | `INTEGER` | да | FK → `master_profiles.user_id` | |
| `service_id` | `INTEGER` | да | FK → `services.id` | |
| `interval_weeks` | `INTEGER` | да | | 1 — еженедельно, 2 — раз в две недели. `CHECK BETWEEN 1 AND 12` |
| `weekday` | `INTEGER` | да | | ISO-8601: 1 — понедельник |
| `time_local` | `TEXT` ЧЧ:ММ | да | | По часам студии, как и график работы |
| `starts_on` | `TEXT` ГГГГ-ММ-ДД | да | | |
| `ends_on` | `TEXT` ГГГГ-ММ-ДД | нет | | Либо дата конца, либо число повторов — что-то одно обязательно |
| `occurrences` | `INTEGER` | нет | | |
| `generate_ahead` | `INTEGER` | да | | На сколько повторов вперёд создавать записи. По умолчанию 3 |
| `is_active` | `INTEGER` 0/1 | да | | |
| `created_by_id` | `INTEGER` | нет | FK → `users.id` | |
| `created_at` | `TEXT` ISO-8601 UTC | да | | |

Записи связаны с серией полями `appointments.series_id` и `occurrence_index`. Занятый повтор пропускается с причиной, а не роняет всю серию: один занятый вторник не должен отменять полгода визитов.

### 4.25 `calendar_feeds` — подписка на календарь

| Поле | Тип | Обяз. | Ключ | Описание |
|---|---|---|---|---|
| `id` | `INTEGER` | да | PK | |
| `user_id` | `INTEGER` | да | FK → `users.id` | |
| `token` | `TEXT` | да | U | Часть адреса ленты |
| `created_at` | `TEXT` ISO-8601 UTC | да | | |
| `revoked_at` | `TEXT` ISO-8601 UTC | нет | | Ссылку можно отозвать, не меняя пароль |
| `last_read_at` | `TEXT` ISO-8601 UTC | нет | | Видно, пользуется ли человек подпиской |

Лента открывается по токену без входа — иначе никак: приложение календаря не умеет логиниться. Поэтому токен длинный и случайный, а отозвать его можно отдельно от пароля.

Отмена по ссылке устроена так же: `appointments.cancel_token` — случайный токен на каждую запись. По номеру записи можно было бы перебором отменять чужие визиты.

### 4.26 `inbox` — уведомления в кабинете

Добавлена миграцией `009_inbox.sql`. Не путать с `notifications` (4.13): та таблица — очередь отправки наружу, у её строк есть канал, время отправки и отметка о доставке, разбирает её отправщик. Эта — то, что человек открывает в кабинете и просматривает.

| Поле | Тип | Обяз. | Ключ | Описание |
|---|---|---|---|---|
| `id` | `INTEGER` | да | PK | |
| `user_id` | `INTEGER` | да | FK → `users.id`, CASCADE | Получатель |
| `kind` | `TEXT` + CHECK | да | | `appointment_cancelled` · `appointment_moved` · `slot_overlapped` |
| `text` | `TEXT` | да | | Готовый текст сообщения |
| `appointment_id` | `INTEGER` | нет | FK → `appointments.id`, SET NULL | Куда ведёт ссылка |
| `read_at` | `TEXT` ISO-8601 UTC | нет | | `NULL` — не прочитано |
| `created_at` | `TEXT` ISO-8601 UTC | да | | |

Индексы: `inbox_user_idx (user_id, created_at DESC)` — список кабинета; `inbox_unread_idx (user_id) WHERE read_at IS NULL` — счётчик в шапке.

**Почему отдельная таблица, а не ещё один `channel` в очереди.** Канал `cabinet` притащил бы в строку поля, у которых для экрана нет смысла: `scheduled_at`, `sent_at`, `error`. Отметку о прочтении пришлось бы прятать в `status`, где уже живут «отправлено» и «не доставлено» — состояния отправщика, а не читателя.

**Почему текст хранится готовым, а не собирается при показе.** Сообщение описывает то, что было в момент события. Запись потом могут перенести ещё раз — старое уведомление должно остаться правдой о прошлом переносе, а не пересчитаться под новое время.

**Список `kind` закрытый и короткий.** Уведомление создаётся только о том, чего человек не делал сам. Собственная запись, отмена и перенос сюда не попадают: он только что видел результат на экране. Напоминания о визите — тоже нет: для них нужен запуск по расписанию, а он живёт в очереди `notifications`.
...
Единственная таблица со словом «slot» в названии — `slot_subscriptions`. Она хранит подписки клиентов на освободившиеся окна конкретного мастера, а не сами окна: строка появляется, когда клиентка включает «Новые окна у Анны», и не зависит от того, свободно сейчас время или нет.
...
| Живые удержания | `slot_holds` при `expires_at > now` |
...
3. **Вычесть блокировки и живые удержания.** Пересекающиеся интервалы из `time_off` — личные и общестудийные — и строки `slot_holds`, у которых `expires_at` ещё не наступил. Истёкшие удержания перед расчётом удаляются.
...
### Пример: Анна, суббота 15 августа, «Маникюр с покрытием»
...
**Разрешённое исключение.** Полное условие срабатывания триггера — `NEW.status IN ('pending','confirmed') AND NEW.allow_overlap = 0`. Признак гасит проверку только для самой создаваемой записи: в подзапросе строки `x` по нему не фильтруются. Поэтому запись, созданная поверх занятого времени, дальше занимает его как обычная, и следующая попытка записать туда же будет отклонена. Ставить признак умеет только администратор — см. журнал разработки, раздел про защиту от двойной записи.

Второй триггер, `appointments_no_overlap_update`, повторяет проверку при изменении `starts_at`, `duration_min`, `master_id`, `status` и `allow_overlap`. Без него запись можно было бы создать на свободное время, а потом перенести на занятое.
...
| `slot_holds (master_id, starts_at)` | Вычитание удержаний при расчёте окон | Каждое открытие календаря перебирает все удержания |
| `slot_holds (expires_at)` | Удаление истёкших удержаний | Уборка перебирает таблицу целиком на каждом расчёте |
...
| Профиль, «Новые окна у Анны» | Булев флаг не хранит, чьих именно окон ждут | Таблица `slot_subscriptions` |
...
Что проверено и не потребовало изменений: кресла студии как ресурс (мастеров трое, кресел четыре — упереться невозможно), несколько услуг за один визит (запись хранит одну услугу, поэтому визит из нескольких становится цепочкой записей подряд), работа через полночь (студия закрывается в 21:00).

**Отдельно про удержание окна.** Здесь решение позже пересмотрено. Сначала «мягкая бронь» была отвергнута: паспорт продукта описывал другое поведение — вторая клиентка получает отказ на подтверждении и обновлённый список окон. Затем удержание вошло в требования к API, и появилась таблица `slot_holds` (раздел 4.22). Отказ на подтверждении никуда не делся — он остаётся последней линией защиты, если удержание истекло, — но теперь до него доходит редко.
