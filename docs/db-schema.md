...
| Границы и шаг | `studio_settings`: `timezone`, `slot_step_min`, `min_lead_time_min`, `booking_horizon_days` |
...
2. **Взять рабочие интервалы.** Если на эту дату есть строка в `schedule_exceptions` — она заменяет недельное правило целиком: при `is_working = false` расчёт закончен, окон нет. Иначе берутся строки `working_hours` на нужный день недели, где дата попадает в `valid_from … valid_to`: у мастера свои, а при `uses_studio_hours = true` — студийные (`master_id IS NULL`). Отсутствие строк означает выходной.
3. **Вычесть блокировки.** Все пересекающиеся интервалы из `time_off` — и личные, и общестудийные.
4. **Вычесть занятое.** Записи этого мастера со статусом `pending` или `confirmed`, пересекающие день, — от `starts_at` до `ends_at`. Отменённые (`cancelled`) время не занимают, завершённые (`done`) — занимают, они в прошлом.
...
**19. Буфер на уборку — свойство услуги, а не общая константа студии.**
...
Что проверено и не потребовало изменений: кресла студии как ресурс (мастеров трое, кресел четыре — упереться невозможно), несколько услуг за один визит (в прототипе запись всегда на одну услугу), работа через полночь (студия закрывается в 21:00).
