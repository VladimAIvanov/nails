...
-- В отличие от сидов, это часть схемы: подписи статусов и единственная строка настроек.
...
INSERT INTO appointment_status_labels (status, title, title_short, color_token, sort_order, show_in_filters) VALUES
  ('pending',   'Ожидает подтверждения', 'Ожидает',     '--honey-500', 10, true),
  ('confirmed', 'Подтверждена',          'Подтверждена', '--clay-600',  20, true),
  ('done',      'Визит состоялся',       'Выполнена',   '--sage-600',  30, false),
  ('cancelled', 'Отменена',              'Отменена',    '--sand-300',  40, true),
  ('no_show',   'Клиент не пришёл',      'Не пришла',   '--sand-500',  50, false);
...
SELECT NULL, d, time '10:00', time '21:00', date '2026-01-01'
